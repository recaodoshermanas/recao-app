...
const NIVEL = { leve: "Leve", grave: "Grave", muy_grave: "Muy grave" };
...
export function MisAvisosView({ user }) {
  ...
  const load = useCallback(async () => { setLoading(true); try { const r = await sb.fn("disciplina", { action: "mis_avisos" }); setAvisos(r.avisos || []); } catch (e) { /* noop */ } setLoading(false); }, []);
  useEffect(() => { load(); }, [load]);
  ...
  return (
    <div style={{ padding: "16px", maxWidth: 540, margin: "0 auto" }}>
      ...
      {loading ? <div style={{ fontFamily: F, fontSize: 13, color: C.mut, textAlign: "center", padding: 24 }}>Cargando…</div>
        : avisos.length === 0 ? <div style={{ fontFamily: SF, fontSize: 16, color: "#1E7A46", textAlign: "center", padding: 30 }}>No tienes ningún aviso ✓</div>
          : avisos.map(a => {
                  ...
                  <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px solid ${C.brdL}` }}>
                    ...
                    {emitido ? (
                      <div style={{ marginTop: 14 }}>
                        ...
                        {alegado ? (
                          <div style={{ background: "#F7F4EE", border: `1px solid ${C.brdL}`, borderRadius: 12, padding: "11px 13px" }}>
                            ...
                            {Array.isArray(a.alegacion_archivos) && a.alegacion_archivos.length > 0 && <div style={{ display: "flex", gap: 6, marginTop: 8, flexWrap: "wrap" }}>{a.alegacion_archivos.map((f, i) => <img key={i} src={f} alt="" style={{ width: 60, height: 60, objectFit: "cover", borderRadius: 8 }} />)}</div>}
                          </div>
                        ) : (
                          <div>
                      ...
                      <div style={{ marginTop: 14, background: "#F7F4EE", border: `1px solid ${C.brdL}`, borderRadius: 12, padding: "11px 13px" }}>
                        ...
                        <div style={{ fontFamily: F, fontSize: 13.5, color: C.char }}>{res ? res.label : a.resultado}{a.nivel ? ` · falta ${NIVEL[a.nivel] || a.nivel}` : ""}{a.articulo ? ` (art. ${a.articulo})` : ""}</div>

...
export function HorariosAdminView() {
  ...
  return (
    <div style={{ padding: "14px", maxWidth: 640, margin: "0 auto" }}>
      <select value={uid} onChange={e => setUid(e.target.value)} style={{ width: "100%", padding: "11px", borderRadius: 10, border: `1.5px solid ${C.brd}`, fontFamily: F, fontSize: 15, background: "#fff", color: C.char, marginBottom: 12 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 4 }}>
        {DIAS.map(d => <div key={d} style={{ textAlign: "center", fontFamily: F, fontSize: 11, fontWeight: 700, color: C.mut, padding: "2px 0" }}>{d}</div>)}
        ...
        {days.map(d => {
          ...
          const esHoy = f === hoy;
          return (
            <button key={f} onClick={() => setPicker(f)} style={{ aspectRatio: "1", border: esHoy ? `2px solid ${C.char}` : `1px solid ${C.brd}`, borderRadius: 8, background: def ? def.bg : "#fff", color: def ? def.fg : C.mut, cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 2, fontFamily: F }}>
              <span style={{ fontSize: 12, fontWeight: 600 }}>{d.getDate()}</span>
      ...
      {picker && (
        <div onClick={() => setPicker(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 50 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: "16px 16px 0 0", padding: 18, width: "100%", maxWidth: 640 }}>
            <div style={{ fontFamily: SF, fontSize: 16, color: C.char, marginBottom: 12 }}>{picker}</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 8 }}>
              {TURNO_OPCIONES.map(t => {
                const def = TURNOS[t];
                return (
                  <button key={t} onClick={() => setTurno(picker, t)} style={{ padding: "12px", borderRadius: 10, border: `1.5px solid ${C.brd}`, background: def.bg, color: def.fg, fontFamily: F, fontSize: 13, fontWeight: 600, cursor: "pointer", textAlign: "left" }}>

...
export function VacacionesAdminView() {
  ...
  const crear = async () => {
    ...
    try { await sb.insert("vacaciones_solicitudes", { usuario_id: nUid, fecha_inicio: nIni, fecha_fin: nFin, dias, estado: nEstado }); setNIni(""); setNFin(""); setNDias(""); setAbrir(false); flash("Periodo añadido"); await load(); } catch (e) { flash(e.message); }
  };
        ...
        {trab.map(t => { const r = resumen[t.id] || { total: 22, restantes: 22 }; return (
          <div key={t.id} style={{ background: "#fff", border: `1px solid ${C.brdL}`, borderRadius: 16, padding: 14, display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              <div style={{ fontFamily: F, fontSize: 12, color: C.mut, marginTop: 1 }}><b style={{ color: C.char, fontFamily: SF, fontSize: 15 }}>{r.restantes}</b> / {r.total} días</div>
            </div>
          </div>
        ); })}
      </div>
